...
export class PedidosdetailsComponent {
  ...
  ngOnInit() {
    this.usuarioService.listAll().subscribe((usuarios: Usuario[]) => {
      this.listaDeUsuarios = usuarios.filter(usuario => usuario.role === 'USER');
    });


    this.usuarioService.listAll().subscribe((usuarios: Usuario[]) => {
      this.listaDeFuncionarios = usuarios.filter(usuario => usuario.role === 'ADMIN');
    });
